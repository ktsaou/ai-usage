...
- **Parked** (`parked: true`): not polled, muted on the dashboard, hidden from the MCP, and `npm run login` neither opens its tab nor waits for its sign-in. Park a browser-session provider whose plan has lapsed rather than removing it: its fetcher and history stay, and resuming is one flag. Nothing is parked today.
