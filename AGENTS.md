...
Provider types:
- **Subscription quota** (zai, minimax, kimi, xai, alibaba; mimo while subscribed): used/total/remaining/percent per window
...
- **Parked** (`parked: true`): not polled, muted on the dashboard, hidden from the MCP, and `npm run login` neither opens its tab nor waits for its sign-in. `mimo` is parked: the plan lapsed, and its fetcher is kept for when it is bought again.
